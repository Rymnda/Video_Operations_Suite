...
ABOUT_SCREENSHOT = ASSETS_DIR / "Screenshot_VideoOperationsSuite.jpg"

AUTHOR_NAME = "Rymnda"
AUTHOR_URL = "https://github.com/Rymnda"
PROJECT_URL = "https://github.com/Rymnda/Video_Operations_Suite"
...
class TimeCodeEdit(QtWidgets.QLineEdit):
    def __init__(self, parent=None):
        super().__init__(parent)
        rx = QtCore.QRegularExpression(r"[0-9:]{0,8}")
        self.setValidator(QtGui.QRegularExpressionValidator(rx, self))
        self.setMaxLength(8)
        self.setText("00:00:00")
        self.editingFinished.connect(self._ensure_not_empty)
...
def load_language_map(lang_dir: Path) -> Dict[str, Dict[str, str]]:
...
class VideoControlBar(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.btn_mark_sec = make_btn(
            "Neem tijd",
            "Gebruik de huidige afspeeltijd als Sec voor geselecteerde bestanden",
            "Ctrl+M",
        )
        self.btn_mark_sec.setFixedWidth(92)
        ...
        self.btn_playpa.setIconSize(QtCore.QSize(32, 32))
        self.btn_playpa.setFixedSize(132, 48)
        self.btn_playpa.setToolTip("Afspelen  •  Space")
        ...
        # goto veld
        self.ed_goto = TimeCodeEdit()
        self.ed_goto.setFixedWidth(92)
        self.ed_goto.setAlignment(QtCore.Qt.AlignCenter)
        self.ed_goto.setToolTip("Formaat: hh:mm:ss, mm:ss of alleen cijfers. Voorbeelden: 2, 200, 0200 of 02:00 = minuut 2.")
    ...
    def set_playing(self, playing: bool):
        ...
        tip = (
            self._tr("vc.pause_tooltip", "Pause")
            if playing
            else self._tr("vc.play_tooltip", "Play")
        )
        self.btn_playpa.setToolTip(tip + "  •  Space")
    ...
    def set_muted_state(self, muted: bool):
        ...
        self.btn_mute.setToolTip(
            self._tr("vc.unmute_tooltip", "Sound on")
            if muted
            else self._tr("vc.mute_tooltip", "Mute")
        )
    ...
    def retranslate_ui(self, trf):
        self._tr = trf
        self.btn_prev.setText(trf("vc.prev", "Previous"))
        self.btn_back30.setText(trf("vc.back30", "-30s"))
        self.btn_back5.setText(trf("vc.back5", "-5s"))
        self.btn_fwd5.setText(trf("vc.fwd5", "+5s"))
        self.btn_fwd30.setText(trf("vc.fwd30", "+30s"))
        self.btn_next.setText(trf("vc.next", "Next"))
        self.btn_mark_sec.setText(trf("vc.take_time", "Take time"))
        self.btn_mark_sec.setToolTip(
            trf("vc.take_time_tooltip", "Use current playback time as Sec for selected files") + "  •  Ctrl+M"
        )
        self.lbl_volume.setText(trf("vc.volume", "Volume"))
        self.vol_slider.setToolTip(trf("vc.volume_tooltip", "Volume"))
        self.ed_goto.setToolTip(trf("vc.goto_tooltip", "Format: hh:mm:ss or mm:ss. Example: 02:00 = minute 2."))
        self.set_playing(False)
        self.set_muted_state(self.vol_slider.value() <= 0)
...
class ImagePreviewDialog(QtWidgets.QDialog):
    def __init__(self, parent, image_path: Path, title: str):
        super().__init__(parent)
        ...
        self.image_label.setStyleSheet("QLabel{background:#0E1217; border:1px solid #2A3945; border-radius:10px;}")
...
class AboutDialog(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        title.setStyleSheet("font-size:22px; color:#EAF7FF;")
        subtitle = QtWidgets.QLabel(f"Version {APP_VER}  |  Modern + Undo/Redo")
        subtitle.setStyleSheet("color:#9DB7C8;")

        info = QtWidgets.QLabel(
            f"Author: <a href='{AUTHOR_URL}'>{AUTHOR_NAME}</a><br>"
            f"Project: <a href='{PROJECT_URL}'>{PROJECT_URL}</a>"
        )
        info.setOpenExternalLinks(True)
        info.setTextFormat(QtCore.Qt.RichText)
        info.setStyleSheet("color:#D9E8F2;")

        self.screenshot = ClickableImageLabel()
        self.screenshot.setAlignment(QtCore.Qt.AlignCenter)
        self.screenshot.setMinimumHeight(420)
        self.screenshot.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
        self.screenshot.setToolTip("Click to open full size")
        self.screenshot.setStyleSheet(
            "QLabel{background:#0E1217; border:1px solid #2A3945; border-radius:12px; padding:8px;}"
        )
        self.screenshot.clicked.connect(self._open_full_preview)
        self._screenshot_pixmap = QtGui.QPixmap(str(ABOUT_SCREENSHOT))
        self._update_screenshot()

        hint = QtWidgets.QLabel("Click the screenshot for full size.")
        hint.setStyleSheet("color:#7F96A5; font-style:italic;")

        btn_row = QtWidgets.QHBoxLayout()
        btn_row.addStretch(1)
        btn_repo = QtWidgets.QPushButton("Open GitHub")
        btn_repo.clicked.connect(lambda: QtGui.QDesktopServices.openUrl(QtCore.QUrl(PROJECT_URL)))
        ...
        root.addWidget(info)
        root.addWidget(self.screenshot, 1)
        root.addWidget(hint)
    ...
    def _update_screenshot(self):
        if self._screenshot_pixmap.isNull():
            self.screenshot.setText("Screenshot unavailable")
            return
        scaled = self._screenshot_pixmap.scaled(
            self.screenshot.size() - QtCore.QSize(16, 16),
            QtCore.Qt.KeepAspectRatio,
            QtCore.Qt.SmoothTransformation,
        )
        self.screenshot.setPixmap(scaled)

    def _open_full_preview(self):
        if self._screenshot_pixmap.isNull():
            return
        dlg = ImagePreviewDialog(self, ABOUT_SCREENSHOT, "Video Operations Suite Screenshot")
        dlg.exec()
...
class Main(QtWidgets.QWidget):
    def __init__(self):
        ...
        self._languages = load_language_map(LANG_DIR)
        ...
        saved_lang = str(self.settings.value("ui/language", "EN")).upper()
        self._lang_code = saved_lang if saved_lang in self._language_names else "EN"
        ...
        for mode in MODE_ORDER:
            self.cmb_mode.addItem(mode, mode)
        ...
        # Middengroep
        top_layout.addSpacing(15)
        self.lbl_sec = QtWidgets.QLabel("Sec:")
        top_layout.addWidget(self.lbl_sec)
        ...
        self.lbl_sort = QtWidgets.QLabel("Sorteer op:")
        self.sortbar.addWidget(self.lbl_sort)
        for b in (
            self.btn_sort_name,
            self.btn_sort_dir,
            self.btn_sort_date,
        ...
        self.lbl_info = QtWidgets.QLabel("Dubbelklik op bestand = Checkbox aan/uit")
        self.lbl_info.setStyleSheet("color:#666; font-style:italic;")
        self.sortbar.addWidget(self.lbl_info)
        ...
        self.apply_language(self._lang_code, save=False)
        self._autofit_start()
    ...
    def tr(self, key: str, default: str = "") -> str:
        if self._lang_code != "EN":
            val = self._languages.get(self._lang_code, {}).get(key)
            if val:
                return val
        return self._languages.get("EN", {}).get(key, default)

    def _translated_headers(self) -> List[str]:
        return [
            self.tr(key, default)
            for key, default in zip(HEADER_KEYS, HEADER)
        ]

    def _mode_label(self, mode: str) -> str:
        key_map = {
            MODE_TRIM_FIRST: "mode.trim_first",
            MODE_TRIM_LAST: "mode.trim_last",
            MODE_ROLL_FIRST: "mode.roll_first",
            MODE_REMUX_COPY: "mode.remux_copy",
            MODE_TRANSCODE: "mode.transcode",
        }
        return self.tr(key_map.get(mode, ""), mode)

    def _retranslate_mode_combo(self):
        current_mode = self.cmb_mode.currentData()
        self.cmb_mode.blockSignals(True)
        self.cmb_mode.clear()
        for mode in MODE_ORDER:
            self.cmb_mode.addItem(self._mode_label(mode), mode)
        idx = self.cmb_mode.findData(current_mode)
        self.cmb_mode.setCurrentIndex(max(0, idx))
        self.cmb_mode.blockSignals(False)

    def apply_language(self, code: str, save: bool = True):
        code = str(code or "EN").upper()
        if code not in self._language_names:
            code = "EN"
        self._lang_code = code
        if save:
            self.settings.setValue("ui/language", code)

        self.setWindowTitle(self.tr("app.title", APP_TITLE))
        self.btn_add.setText(self.tr("button.add_file", "➕ File"))
        self.btn_add.setToolTip(self.tr("tooltip.add_file", "Add file"))
        self.btn_add_dir.setText(self.tr("button.add_folder", "📂 Folder"))
        self.btn_add_dir.setToolTip(self.tr("tooltip.add_folder", "Add folder"))
        self.btn_refresh.setText(self.tr("button.refresh", "Refresh"))
        self.btn_refresh.setToolTip(self.tr("tooltip.refresh", "Refresh list"))
        self.btn_autofit.setText(self.tr("button.autofit", "Autofit"))
        self.btn_autofit.setToolTip(self.tr("tooltip.autofit", "Fit columns automatically"))
        self.btn_remove.setText(self.tr("button.remove", "🗑 Remove"))
        self.btn_remove.setToolTip(self.tr("tooltip.remove", "Remove selected"))
        self.btn_clear.setText(self.tr("button.clear", "🧹 Clear list"))
        self.btn_clear.setToolTip(self.tr("tooltip.clear", "Clear entire list"))
        self.btn_undo.setText(self.tr("button.undo", "Undo"))
        self.btn_undo.setToolTip(self.tr("tooltip.undo", "Undo (Ctrl+Z)"))
        self.btn_redo.setText(self.tr("button.redo", "Redo"))
        self.btn_redo.setToolTip(self.tr("tooltip.redo", "Redo (Ctrl+Y)"))
        self.lbl_search.setText(self.tr("label.search", "Search:"))
        self.ed_filter.setPlaceholderText(self.tr("placeholder.filter", "Filter name..."))
        self.spin_secs.setToolTip(self.tr("tooltip.default_secs", "Default seconds for new items"))
        self.lbl_sec.setText(self.tr("label.sec", "Sec:"))
        self.btn_apply.setText(self.tr("button.apply", "Apply"))
        self.btn_apply.setToolTip(self.tr("tooltip.apply", "Apply seconds to selected rows"))
        self.btn_detect.setText(self.tr("button.detect", "Intro detect"))
        self.btn_detect.setToolTip(self.tr("tooltip.detect", "Automatically detect intro in selected files"))
        self.btn_thumb_edit.setText(self.tr("button.thumb_editor", "📋 Thumbnail Editor"))
        self.btn_thumb_edit.setToolTip(self.tr("tooltip.thumb_editor", "Manually pick a thumbnail"))
        self.chk_cuda.setText(self.tr("checkbox.cuda", "CUDA"))
        self.chk_overwrite.setText(self.tr("checkbox.overwrite", "Overwrite original"))
        self.chk_overwrite.setToolTip(self.tr("tooltip.overwrite", "Creates a .bak and replaces the original"))
        self.chk_fallback.setText(self.tr("checkbox.fallback", "Fallback"))
        self.chk_fallback.setToolTip(self.tr("tooltip.fallback", "Fallback to CPU if GPU fails"))
        self.btn_start.setText(self.tr("button.start", "Start"))
        self.btn_start.setToolTip(self.tr("tooltip.start", "Start batch"))
        self.btn_pause_all.setText(self.tr("button.pause", "Pause"))
        self.btn_pause_all.setToolTip(self.tr("tooltip.pause", "Pause batch"))
        self.btn_resume_all.setText(self.tr("button.resume", "Resume"))
        self.btn_resume_all.setToolTip(self.tr("tooltip.resume", "Resume batch"))
        self.btn_stop_all.setText(self.tr("button.stop", "Stop"))
        self.btn_stop_all.setToolTip(self.tr("tooltip.stop", "Stop all"))
        self.lbl_overall.setText(self.tr("label.ready", "Ready"))
        self.lbl_sort.setText(self.tr("label.sort_by", "Sort by:"))
        self.lbl_info.setText(self.tr("label.doubleclick_toggle", "Double-click file = checkbox on/off"))
        self.btn_sort_name.setText(self.tr("sort.name", "Name"))
        self.btn_sort_dir.setText(self.tr("sort.dir", "Folder"))
        self.btn_sort_date.setText(self.tr("sort.date", "Date"))
        self.btn_sort_size.setText(self.tr("sort.size", "Size"))
        self.btn_sort_type.setText(self.tr("sort.type", "Type"))
        self.btn_sort_reset.setText(self.tr("sort.reset", "Reset"))
        self.table.setHorizontalHeaderLabels(self._translated_headers())
        self._retranslate_mode_combo()
        self.vc.retranslate_ui(self.tr)
        self.menu_bar.clear()
        self._build_menubar(self.menu_bar)

    # ---------------- menubar ----------------
    def _build_menubar(self, mb: QtWidgets.QMenuBar):
        # Bestand
        m_file = mb.addMenu(self.tr("menu.file", "File"))
        a_open = m_file.addAction(self.tr("action.open_files", "Open files..."))
        a_open.triggered.connect(self.on_browse)

        a_open_dir = m_file.addAction(self.tr("action.open_folder", "Open folder..."))
        a_open_dir.triggered.connect(self.on_browse_folder)

        m_file.addSeparator()

        a_saveq = m_file.addAction(self.tr("action.save_session", "Save session (.vos)..."))
        a_saveq.triggered.connect(self._save_session_dialog)

        a_loadq = m_file.addAction(self.tr("action.load_session", "Open session (.vos)..."))
        a_loadq.triggered.connect(self._load_session_dialog)

        m_file.addSeparator()

        a_exit = m_file.addAction(self.tr("action.exit", "Exit"))
        a_exit.triggered.connect(self.close)

        # Bewerken (Undo/Redo)
        m_edit = mb.addMenu(self.tr("menu.edit", "Edit"))
        a_undo = m_edit.addAction(self.tr("action.undo", "Undo"))
        a_undo.setShortcut("Ctrl+Z")
        a_undo.triggered.connect(self.undo_action)

        a_redo = m_edit.addAction(self.tr("action.redo", "Redo"))
        a_redo.setShortcut("Ctrl+Y")
        a_redo.triggered.connect(self.redo_action)

        # Instellingen
        m_view = mb.addMenu(self.tr("menu.settings", "Settings"))

        self.a_del_src = m_view.addAction(self.tr("action.delete_source", "Delete original after success"))
        ...
        self.a_auto_clear = m_view.addAction(self.tr("action.auto_clear", "Clear list after success"))
        ...
        m_view.addSeparator()
        m_lang = m_view.addMenu(self.tr("menu.language", "Language"))
        lang_group = QtGui.QActionGroup(self)
        lang_group.setExclusive(True)
        for code_opt in ("EN", "NL", "ES", "FR", "DE"):
            action = m_lang.addAction(self.tr(f"lang.{code_opt.lower()}", self._language_names[code_opt]))
            action.setCheckable(True)
            action.setChecked(code_opt == self._lang_code)
            action.triggered.connect(lambda checked=False, c=code_opt: self.apply_language(c))
            lang_group.addAction(action)

        m_view.addSeparator()
        a_settings = m_view.addAction(self.tr("action.full_settings", "Full settings..."))
        a_settings.triggered.connect(self.on_settings)

        # Help
        m_help = mb.addMenu(self.tr("menu.help", "Help"))
        a_about = m_help.addAction(self.tr("action.about", "About..."))
            ...
            jobs.append(
                {
                    "row": r,
                    "task": {
                        "mode": self.cmb_mode.currentData() or self.cmb_mode.currentText(),
                        "src": str(src),
                        "out": str(out),
                        "secs": secs,
